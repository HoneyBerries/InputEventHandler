...
- **Architecture:**
  ...
  - `server.go`: TCP server management, connection lifecycle, and JSON request/response handling.
  - `input.go`: Low-level Win32 wrappers for `keybd_event` and `mouse_event`.
  - `keymap.go`: Mapping of string Virtual Key names (e.g., `VK_A`, `VK_LBUTTON`) to their numeric codes.
...
- **Test:** No automated tests currently exist in the codebase. TODO: Implement unit tests for key mapping and server logic.
...
- **Error Handling:** 
  - Errors are logged to `stdout` using the `slog` package.
...
- `server.go`: Main server logic and protocol handling.
- `input.go`: Win32 syscall implementation for input events.
- `keymap.go`: The source of truth for supported Virtual Key names.
